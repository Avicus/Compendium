...
public abstract class Countdown {
  ...
  protected final BossBar getBossBar(Player player) {
    return this.bars.computeIfAbsent(player.getUniqueId(),
        uniqueId -> CompendiumPlugin.getInstance().getBossBarManager().create(player));
  }
  ...
  protected void updateBossBar(Localizable name, Duration elapsed) {
    final float percent = (float) 1 - ((float) elapsed.getStandardSeconds() / (float) this.duration
        .getStandardSeconds());
