...
public class UtilityCommands {
  ...
  @Command(aliases = {
      "exec"}, desc = "Run a command N times.", usage = "<count> <command>", min = 2)
  ...
  @Command(aliases = {"execdelay",
      "ed"}, desc = "Run a command N times with a delay.", usage = "<count> <delay> <command>", min = 2)
  public static void execDelayed(CommandContext args, CommandSender sender)
  ...
  public static void safeStop(CommandContext cmd, CommandSender sender) throws CommandException {
    ...
    if (Bukkit.getOnlinePlayers().size() - offset == 0) {
      ...
      sender.sendMessage(
          ChatColor.RED + "The server cannot be stopped because there are players on it.");
    }
  }
}
